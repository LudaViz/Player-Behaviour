...
#include UE_INLINE_GENERATED_CPP_BY_NAME(CustomProceduralMeshComponent)
...
void UCustomProceduralMeshComponent::PostLoad()
{
	Super::PostLoad();
...
void UCustomProceduralMeshComponent::ClearMeshSection(int32 SectionIndex)
{
	if (SectionIndex < ProcMeshSections.Num())
	{
...
void UCustomProceduralMeshComponent::ClearAllMeshSections()
{
...
void UCustomProceduralMeshComponent::SetMeshSectionVisible(int32 SectionIndex, bool bNewVisibility)
{
	if(SectionIndex < ProcMeshSections.Num())
	{
...
bool UCustomProceduralMeshComponent::IsMeshSectionVisible(int32 SectionIndex) const
{
	return (SectionIndex < ProcMeshSections.Num()) ? ProcMeshSections[SectionIndex].bSectionVisible : false;
}

int32 UCustomProceduralMeshComponent::GetNumSections() const
{
	return ProcMeshSections.Num();
}

void UCustomProceduralMeshComponent::AddCollisionConvexMesh(TArray<FVector> ConvexVerts)
{
...
void UCustomProceduralMeshComponent::ClearCollisionConvexMeshes()
{
...
void UCustomProceduralMeshComponent::SetCollisionConvexMeshes(const TArray< TArray<FVector> >& ConvexMeshes)
{
...
void UCustomProceduralMeshComponent::UpdateLocalBounds()
{
...
FProcMeshSection* UCustomProceduralMeshComponent::GetProcMeshSection(int32 SectionIndex)
{
	if (SectionIndex < ProcMeshSections.Num())
	{
...
void UCustomProceduralMeshComponent::SetProcMeshSection(int32 SectionIndex, const FProcMeshSection& Section)
{
	// Ensure sections array is long enough
	if (SectionIndex >= ProcMeshSections.Num())
	{
...
bool UCustomProceduralMeshComponent::GetTriMeshSizeEstimates(struct FTriMeshCollisionDataEstimates& OutTriMeshEstimates, bool bInUseAllTriData) const
{
	for (const FProcMeshSection& Section : ProcMeshSections)
	{
		if (Section.bEnableCollision)
...
UBodySetup* UCustomProceduralMeshComponent::CreateBodySetupHelper()
{
...
void UCustomProceduralMeshComponent::CreateProcMeshBodySetup()
{
...
void UCustomProceduralMeshComponent::UpdateCollision()
{
	...
	if(bUseAsyncCook)
	{
		UseBodySetup->CreatePhysicsMeshesAsync(FOnAsyncPhysicsCookFinished::CreateUObject(this, &UCustomProceduralMeshComponent::FinishPhysicsAsyncCook, UseBodySetup));
	}
	else
	{
...
void UCustomProceduralMeshComponent::FinishPhysicsAsyncCook(bool bSuccess, UBodySetup* FinishedBodySetup)
{
...
UMaterialInterface* UCustomProceduralMeshComponent::GetMaterialFromCollisionFaceIndex(int32 FaceIndex, int32& SectionIndex) const
{
